...
class Student(IdBaseTable, table=True):
    __table_args__ = (
        UniqueConstraint("name", "phone", name="name_phone"),
    )
    teacher_id: int = Field(foreign_key="teacher.id")  # Define the foreign key
...
class Assignment(IdBaseTable, table=True):
    __table_args__ = (
        UniqueConstraint("id", "title", name="id_title"),
    )
    student_id: int = Field(foreign_key="student.id")  # Define the foreign key
