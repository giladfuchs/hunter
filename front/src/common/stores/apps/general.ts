...
export const useGeneralStore = defineStore({
    id: 'general',
    getters: {
        inputFilters_to_data: () => (form: any, other_params = {}, exclude_field: string[] = []): number => {
            return Object.assign(other_params, ...Object.keys(form).filter((key: string) => form[key].value !== undefined && form[key].value != '' && !exclude_field.includes(key)).map((key: string) => {
    ...
    actions: {
        ...
        async create_update_row_inputFilters(form: any, model: FilterFetchType | string, key_success: string | undefined = undefined, other_params = {}, exclude_field: string[] = []) {
            const data: any = this.inputFilters_to_data(form, other_params, exclude_field)
            if (model.includes("student"))
