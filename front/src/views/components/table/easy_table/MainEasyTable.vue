...
const listCards: any = computed(() => {
  return props.rows !== undefined ? props.rows.map((r: any) => {
    return {...r, key: r.id}
  }) : [];
});
      ...
      <UiParentCard>
        <v-row justify="space-between"
          ...
          <v-col class="d-flex flex-row" cols="3" md="3">
            <v-btn v-if="props.model===FilterFetchType.teacher" :href="`${props.model}/add`" rounded="md"
                   active-color="secondary" icon color="secondary" variant="text">
