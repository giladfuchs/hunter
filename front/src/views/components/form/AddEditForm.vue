...
const controls = computed<inputFilters>(() =>
  create_input_filter(
    ...
    url.value !== 'add' ? array_obj_to_obj_with_key(store.list[model.value] as [], +url.value, 'id') : {}
  )
);
...
const create_update = async (data: inputFilters) => {
  ...
  if (model.value === FilterFetchType.teacher) await router.push('/login');
  else if (model.value === FilterFetchType.student && url.value === 'add') await router.push(`/${FilterFetchType.student}`);
  else await router.push(`/${FilterFetchType.student}/view/${url.value}`);
};
</script>
<template>
  <v-row>
