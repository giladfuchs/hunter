...
import FormChild from '@/views/components/form/FormChild.vue';

import {useGeneralStore} from '@/common/stores/general';
import {create_form_fields} from '@/common/utils/transformation/form';
...
const obj = is_add ? {} : array_obj_to_obj_with_key(store.list[model.value] as [], +url.value, 'id');
...
<template>
  <v-row>
    <v-col class="d-flex flex-row mt-2 justify-space-around" cols="12" sm="12">
      <FormChild :is_add="url === 'add'" :type="model" :controls="controls" @sendForm="create_update"/>
    </v-col>
  </v-row>
</template>
